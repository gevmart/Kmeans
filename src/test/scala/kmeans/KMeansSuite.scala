...
class KMeansSuite extends FunSuite {
  ...
  def checkUpdate(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point], expected: GenSeq[Point]): Unit = {
    assert(update(classified, oldMeans) == expected,
      s"update($classified, $oldMeans should equal to $expected")
  }

  test("update without parallelism should work correctly") {
    val p0 = new Point(0, 0, 0)
    val p3 = new Point(3, 3, 3)
    val classified = GenMap[Point, GenSeq[Point]](p0 -> GenSeq(new Point(1, 1, 1)),
      p3 -> GenSeq(new Point(4, 4, 4), new Point(5, 5, 5), new Point(6, 6, 6)))
    val oldMeans = GenSeq(new Point(3, 3, 3), new Point(0, 0, 0))
    val expected = GenSeq(new Point(5, 5, 5), new Point(1, 1, 1))
